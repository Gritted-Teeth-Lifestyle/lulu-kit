"""
winvision.py — Windows desktop vision + control tool for Claude
===============================================================
Gives Claude sight and control over any Windows app.

COMMANDS:
  screenshot [WindowTitle]          Full screen or specific window
  bgshot WindowTitle                Capture window even if hidden behind others
  region x1 y1 x2 y2               Capture a specific screen region
  zoom x1 y1 x2 y2 [scale]         Crop + enlarge a region (default scale=3)
  crop x1 y1 x2 y2                 Crop from last screenshot (relative coords)
  record [WindowTitle] [fps] [dur]  Record motion — contact sheet of frames
                                    fps default=2, dur default=4s, cols=4
                                    e.g. record "Chrome" 3 6
  click x y                        Left-click
  rclick x y                       Right-click
  dclick x y                       Double-click
  move x y                         Move mouse (no click)
  drag x1 y1 x2 y2 [duration]      Click-drag from (x1,y1) to (x2,y2)
  scroll x y amount                Scroll at position (positive=up, negative=down)
  type "text"                      Type text (Unicode safe via clipboard)
  paste "text"                     Set clipboard and press Ctrl+V
  hotkey ctrl c                    Press key combo (e.g. hotkey ctrl s)
  key enter                        Press a single key
  wait seconds                     Sleep for N seconds
  windows                          List all open window titles + positions
  focus WindowTitle                Bring window to front
  info WindowTitle                 Print window position/size

OUTPUT: all screenshots saved to screen.png (same folder as this script)
"""

import os
import sys
import time
import subprocess
import ctypes
import ctypes.wintypes
import pyautogui
import pygetwindow as gw
from PIL import ImageGrab, Image

_HERE = os.path.dirname(os.path.abspath(__file__))
SCREENSHOT_PATH = os.path.join(_HERE, "screen.png")

pyautogui.FAILSAFE = True
pyautogui.PAUSE = 0.1

# Target width Claude can read clearly. 4K -> 1920 cuts each pixel to half,
# making text readable. Zoom/crop commands bypass this to preserve their upscale.
CLAUDE_MAX_WIDTH = 1920


# ── Resize helper ──────────────────────────────────────────────────────────────

def _normalize(img, max_width=CLAUDE_MAX_WIDTH):
    """Downscale wide images so Claude can read them clearly. Never upscales."""
    if img.width > max_width:
        ratio = max_width / img.width
        new_size = (max_width, int(img.height * ratio))
        img = img.resize(new_size, Image.LANCZOS)
    return img


def _save(img, label="", normalize=True):
    if normalize:
        orig = img.size
        img = _normalize(img)
        if img.size != orig:
            print(f"Resized: {orig} to {img.size}  (normalized for Claude)")
    img.save(SCREENSHOT_PATH)
    msg = f"Saved: {SCREENSHOT_PATH}  ({img.size[0]}x{img.size[1]})"
    print(f"{label}  {msg}" if label else msg)
    return img


# ── Background window capture (PrintWindow API) ────────────────────────────────

def _find_window(title_fragment):
    """Find the first window whose title contains title_fragment (case-insensitive)."""
    for w in gw.getAllWindows():
        safe = w.title.encode('ascii', errors='replace').decode('ascii')
        if title_fragment.lower() in safe.lower() and w.width > 0 and w.height > 0:
            return w
    return None


def capture_window_bg(window_title):
    """
    Capture a window's content even when it's hidden behind other windows.
    Uses the Windows PrintWindow API — renders the window's own bitmap directly.
    Finds the first window whose title contains window_title (partial, case-insensitive).
    """
    win = _find_window(window_title)
    if not win:
        print(f"ERROR: No window found matching '{window_title}'")
        sys.exit(1)

    import win32gui
    import win32ui
    import win32con

    hwnd = win32gui.FindWindow(None, win.title)
    if not hwnd:
        # Fallback: enumerate all windows for partial match
        def _enum(h, result):
            t = win32gui.GetWindowText(h)
            if window_title.lower() in t.lower() and win32gui.IsWindowVisible(h):
                result.append(h)
        handles = []
        win32gui.EnumWindows(_enum, handles)
        if not handles:
            print(f"ERROR: Could not get HWND for '{window_title}'")
            sys.exit(1)
        hwnd = handles[0]

    left, top, right, bottom = win32gui.GetWindowRect(hwnd)
    w = right - left
    h = bottom - top

    hwnd_dc = win32gui.GetWindowDC(hwnd)
    mfc_dc = win32ui.CreateDCFromHandle(hwnd_dc)
    save_dc = mfc_dc.CreateCompatibleDC()
    bmp = win32ui.CreateBitmap()
    bmp.CreateCompatibleBitmap(mfc_dc, w, h)
    save_dc.SelectObject(bmp)

    # PW_RENDERFULLCONTENT (2) renders layered/GPU-accelerated windows correctly
    result = ctypes.windll.user32.PrintWindow(hwnd, save_dc.GetSafeHdc(), 2)
    if not result:
        # Fallback to PW_CLIENTONLY (1)
        ctypes.windll.user32.PrintWindow(hwnd, save_dc.GetSafeHdc(), 1)

    bmp_info = bmp.GetInfo()
    bmp_str = bmp.GetBitmapBits(True)

    img = Image.frombuffer(
        'RGB',
        (bmp_info['bmWidth'], bmp_info['bmHeight']),
        bmp_str, 'raw', 'BGRX', 0, 1
    )

    win32gui.DeleteObject(bmp.GetHandle())
    save_dc.DeleteDC()
    mfc_dc.DeleteDC()
    win32gui.ReleaseDC(hwnd, hwnd_dc)

    safe_title = win.title.encode('ascii', errors='replace').decode('ascii')
    print(f"BgShot '{safe_title}' ({w}x{h}) via PrintWindow")
    return _save(img)


# ── Screenshot helpers ─────────────────────────────────────────────────────────

def screenshot(window_title=None):
    if window_title:
        matches = gw.getWindowsWithTitle(window_title)
        if not matches:
            print(f"ERROR: No window found matching '{window_title}'")
            print("Run: python winvision.py windows")
            sys.exit(1)
        win = matches[0]
        try:
            win.activate()
            time.sleep(0.4)
        except Exception:
            pass
        bbox = (win.left, win.top, win.right, win.bottom)
        img = ImageGrab.grab(bbox=bbox)
        print(f"Window '{win.title}' at {bbox}  raw={img.size}")
    else:
        img = ImageGrab.grab()
        print(f"Full screen raw: {img.size}")

    return _save(img)


def region(x1, y1, x2, y2):
    """Capture a specific rectangle of the screen."""
    img = ImageGrab.grab(bbox=(x1, y1, x2, y2))
    print(f"Region ({x1},{y1})->({x2},{y2})  raw={img.size}")
    return _save(img)


def zoom(x1, y1, x2, y2, scale=3):
    """Capture a screen region and scale it up for readability. Skips normalization."""
    img = ImageGrab.grab(bbox=(x1, y1, x2, y2))
    new_size = (img.width * scale, img.height * scale)
    img = img.resize(new_size, Image.NEAREST)
    # Zoom is intentionally upscaled — don't normalize back down
    return _save(img, label=f"Zoom {scale}x  ({x1},{y1})->({x2},{y2})", normalize=False)


def crop(x1, y1, x2, y2, scale=2):
    """Crop from the last saved screenshot (pixel coords in that image) and upscale."""
    try:
        img = Image.open(SCREENSHOT_PATH)
    except FileNotFoundError:
        print("ERROR: No screenshot saved yet. Run 'screenshot' first.")
        sys.exit(1)
    cropped = img.crop((x1, y1, x2, y2))
    new_size = (cropped.width * scale, cropped.height * scale)
    cropped = cropped.resize(new_size, Image.LANCZOS)
    # Crop is intentionally upscaled — don't normalize back down
    _save(cropped, label=f"Crop ({x1},{y1})->({x2},{y2}) scaled {scale}x", normalize=False)


# ── Record (motion contact sheet) ─────────────────────────────────────────────

def record(window_title=None, fps=2, duration=4, cols=4):
    """
    Capture frames at `fps` for `duration` seconds, stitch into a contact
    sheet grid so Claude can see the full motion in one image.
    Each frame is labelled with its timestamp.
    """
    from PIL import ImageDraw, ImageFont

    interval = 1.0 / fps
    total_frames = max(1, int(fps * duration))

    # Resolve capture bbox once
    bbox = None
    win_label = "full screen"
    if window_title:
        matches = gw.getWindowsWithTitle(window_title)
        if not matches:
            print(f"ERROR: No window matching '{window_title}'")
            sys.exit(1)
        win = matches[0]
        try:
            win.activate()
            time.sleep(0.3)
        except Exception:
            pass
        bbox = (win.left, win.top, win.right, win.bottom)
        win_label = win.title

    print(f"Recording '{win_label}'  {fps}fps x {duration}s = {total_frames} frames ...")

    frames = []
    timestamps = []
    start = time.time()
    for i in range(total_frames):
        frame_start = time.time()
        img = ImageGrab.grab(bbox=bbox)
        img = _normalize(img)   # normalize each frame individually
        frames.append(img)
        timestamps.append(time.time() - start)
        print(f"  frame {i+1}/{total_frames}  t={timestamps[-1]:.2f}s  size={img.size}")
        # Sleep just enough to hit target fps
        elapsed = time.time() - frame_start
        gap = interval - elapsed
        if gap > 0:
            time.sleep(gap)

    # Build contact sheet
    pad = 4
    # Scale each frame to thumbnail size so the sheet stays readable (~1800px wide)
    thumb_w = min(frames[0].size[0], (1800 - pad * (cols + 1)) // cols)
    ratio = thumb_w / frames[0].size[0]
    thumb_h = int(frames[0].size[1] * ratio)
    frames = [f.resize((thumb_w, thumb_h), Image.LANCZOS) for f in frames]

    fw, fh = frames[0].size
    rows = (len(frames) + cols - 1) // cols
    label_h = 20
    sheet_w = cols * (fw + pad) + pad
    sheet_h = rows * (fh + label_h + pad) + pad

    sheet = Image.new("RGB", (sheet_w, sheet_h), (30, 30, 30))
    draw = ImageDraw.Draw(sheet)

    # Try to load a small font, fall back to default
    try:
        font = ImageFont.truetype("arial.ttf", 14)
    except Exception:
        font = ImageFont.load_default()

    for idx, (frame, ts) in enumerate(zip(frames, timestamps)):
        col = idx % cols
        row = idx // cols
        x = pad + col * (fw + pad)
        y = pad + row * (fh + label_h + pad)
        sheet.paste(frame, (x, y))
        label = f"t={ts:.2f}s"
        draw.rectangle([x, y + fh, x + fw, y + fh + label_h], fill=(30, 30, 30))
        draw.text((x + 4, y + fh + 2), label, fill=(200, 200, 200), font=font)

    sheet.save(SCREENSHOT_PATH)
    size_kb = os.path.getsize(SCREENSHOT_PATH) // 1024
    print(f"Contact sheet: {cols}x{rows} grid  {sheet_w}x{sheet_h}px  {size_kb}KB")
    print(f"Saved: {SCREENSHOT_PATH}")


# ── Window helpers ─────────────────────────────────────────────────────────────

def list_windows():
    wins = gw.getAllWindows()
    visible = [w for w in wins if w.title.strip() and w.width > 0]
    print(f"Open windows ({len(visible)}):")
    for w in sorted(visible, key=lambda x: x.title):
        safe = w.title.encode('ascii', errors='replace').decode('ascii')
        print(f"  [{w.left:5},{w.top:5}] {w.width:4}x{w.height:4}  {safe}")


def focus_window(title):
    matches = gw.getWindowsWithTitle(title)
    if not matches:
        print(f"ERROR: No window matching '{title}'")
        sys.exit(1)
    win = matches[0]
    win.activate()
    print(f"Focused: '{win.title}'")


def window_info(title):
    matches = gw.getWindowsWithTitle(title)
    if not matches:
        print(f"ERROR: No window matching '{title}'")
        sys.exit(1)
    win = matches[0]
    print(f"Title : {win.title}")
    print(f"Pos   : left={win.left}  top={win.top}")
    print(f"Size  : {win.width} x {win.height}")
    print(f"Box   : ({win.left},{win.top}) to ({win.right},{win.bottom})")


# ── Clipboard helper ──────────────────────────────────────────────────────────

def _set_clipboard(text):
    """Set clipboard text using PowerShell (handles Unicode)."""
    escaped = text.replace("'", "''")
    subprocess.run(
        ["powershell", "-Command", f"Set-Clipboard -Value '{escaped}'"],
        check=True, capture_output=True
    )


# ── Main ───────────────────────────────────────────────────────────────────────

def main():
    if len(sys.argv) < 2:
        print(__doc__)
        sys.exit(0)

    cmd = sys.argv[1].lower()

    if cmd == "screenshot":
        title = " ".join(sys.argv[2:]) if len(sys.argv) > 2 else None
        screenshot(title)

    elif cmd == "bgshot":
        if len(sys.argv) < 3:
            print("Usage: bgshot WindowTitle")
            sys.exit(1)
        title = " ".join(sys.argv[2:])
        capture_window_bg(title)

    elif cmd == "record":
        # record [WindowTitle] [fps] [duration] [cols]
        # Args are positional but WindowTitle can be multi-word, so detect numbers
        rest = sys.argv[2:]
        # Pull trailing numeric args off the end
        cols = 4
        duration = 4.0
        fps = 2.0
        title_parts = []
        num_args = []
        for a in rest:
            try:
                num_args.append(float(a))
            except ValueError:
                title_parts.append(a)
        if len(num_args) >= 1: fps = num_args[0]
        if len(num_args) >= 2: duration = num_args[1]
        if len(num_args) >= 3: cols = int(num_args[2])
        title = " ".join(title_parts) if title_parts else None
        record(title, fps=fps, duration=duration, cols=cols)

    elif cmd == "region":
        x1, y1, x2, y2 = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])
        region(x1, y1, x2, y2)

    elif cmd == "zoom":
        x1, y1, x2, y2 = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])
        scale = int(sys.argv[6]) if len(sys.argv) > 6 else 3
        zoom(x1, y1, x2, y2, scale)

    elif cmd == "crop":
        x1, y1, x2, y2 = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])
        scale = int(sys.argv[6]) if len(sys.argv) > 6 else 2
        crop(x1, y1, x2, y2, scale)

    elif cmd == "click":
        x, y = int(sys.argv[2]), int(sys.argv[3])
        pyautogui.click(x, y)
        print(f"Clicked ({x},{y})")

    elif cmd == "rclick":
        x, y = int(sys.argv[2]), int(sys.argv[3])
        pyautogui.rightClick(x, y)
        print(f"Right-clicked ({x},{y})")

    elif cmd == "dclick":
        x, y = int(sys.argv[2]), int(sys.argv[3])
        pyautogui.doubleClick(x, y)
        print(f"Double-clicked ({x},{y})")

    elif cmd == "move":
        x, y = int(sys.argv[2]), int(sys.argv[3])
        pyautogui.moveTo(x, y)
        print(f"Moved to ({x},{y})")

    elif cmd == "scroll":
        x, y, amount = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4])
        pyautogui.scroll(amount, x=x, y=y)
        print(f"Scrolled {amount} at ({x},{y})")

    elif cmd == "drag":
        x1, y1, x2, y2 = int(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]), int(sys.argv[5])
        duration = float(sys.argv[6]) if len(sys.argv) > 6 else 0.3
        pyautogui.moveTo(x1, y1)
        pyautogui.dragTo(x2, y2, duration=duration, button='left')
        print(f"Dragged ({x1},{y1}) -> ({x2},{y2}) over {duration}s")

    elif cmd == "type":
        text = " ".join(sys.argv[2:])
        # Use clipboard for Unicode safety
        _set_clipboard(text)
        pyautogui.hotkey('ctrl', 'v')
        print(f"Typed (via clipboard): {text}")

    elif cmd == "paste":
        text = " ".join(sys.argv[2:])
        _set_clipboard(text)
        pyautogui.hotkey('ctrl', 'v')
        print(f"Pasted: {text}")

    elif cmd == "hotkey":
        keys = sys.argv[2:]
        pyautogui.hotkey(*keys)
        print(f"Hotkey: {'+'.join(keys)}")

    elif cmd == "key":
        key = sys.argv[2]
        pyautogui.press(key)
        print(f"Pressed: {key}")

    elif cmd == "wait":
        secs = float(sys.argv[2]) if len(sys.argv) > 2 else 1.0
        time.sleep(secs)
        print(f"Waited {secs}s")

    elif cmd == "windows":
        list_windows()

    elif cmd == "focus":
        title = " ".join(sys.argv[2:])
        focus_window(title)

    elif cmd == "info":
        title = " ".join(sys.argv[2:])
        window_info(title)

    else:
        print(f"Unknown command: {cmd}")
        print(__doc__)
        sys.exit(1)


if __name__ == "__main__":
    main()
