...
OPTIONS:
  ...
  --mobile                         Simulate iPhone 14 (390x844, touch, user agent)
  --ls KEY=VALUE [KEY=VALUE ...]   Set localStorage keys before navigating (open only)
  --wait N                         Wait N seconds after page load before screenshotting
...
STATE_PATH = os.path.join(_HERE, "browser_state.json")

# Parse global flags before command
def parse_flags(args):
    headed = "--headed" in args
    mobile = "--mobile" in args
    fullpage = "--fullpage" in args
    wait_secs = 0.0
    if "--wait" in args:
        idx = args.index("--wait")
        if idx + 1 < len(args):
            try: wait_secs = float(args[idx + 1])
            except ValueError: pass
    # Extract --ls KEY=VALUE pairs
    ls_items = {}
    filtered = []
    i = 0
    while i < len(args):
        if args[i] == "--ls" and i + 1 < len(args):
            i += 1
            ...
            i += 2  # skip --wait and its value
        elif args[i] not in ("--headed", "--mobile", "--fullpage"):
            filtered.append(args[i])
            i += 1
        else:
            i += 1
    return filtered, headed, mobile, fullpage, wait_secs, ls_items
...
def run(args_raw):
    ...
    args, headed, mobile, fullpage, wait_secs, ls_items = parse_flags(args_raw)
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            **(IPHONE_14 if mobile else {"viewport": {"width": 1920, "height": 1080}, "device_scale_factor": 1}),
            ignore_https_errors=True,
        )
        page = context.new_page()

        # Always restore localStorage from state (persists across open/navigate calls)
        _saved_local_storage = {}
        if os.path.exists(STATE_PATH):
            with open(STATE_PATH) as f:
                _state = json.load(f)
            _saved_local_storage = _state.get("local_storage", {})

        if _saved_local_storage:
            init_script = " ".join(
                f"localStorage.setItem({repr(k)}, {repr(v)});"
                for k, v in _saved_local_storage.items()
            )
            context.add_init_script(init_script)

        # Restore session URL if not explicitly opening a new one
        ...
        try:
            if cmd == "open":
                ...
                # If localStorage keys requested, navigate to origin first to set them
                if ls_items:
                    ...
                    for k, v in ls_items.items():
                        ...
                        print(f"localStorage: {k} = {v}")
                page.goto(url, wait_until="domcontentloaded", timeout=30000)
                # Re-inject saved localStorage in case redirect cleared it
                if _saved_local_storage:
                    page.evaluate(" ".join(
                        f"localStorage.setItem({repr(k)}, {repr(v)});"
                        for k, v in _saved_local_storage.items()
